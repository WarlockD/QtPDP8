...
inline int ea (size_t addr, int16_t opcode, bool& indirect, bool& curpage)
{
...
inline std::string to_octal(int num,const char* fmt="%04o") {
    char buff[8];
    sprintf(buff,fmt,num);
    return buff;
}
bool fetch_iot (int code, std::string* dis, std::string* com)
{
...
void simple_dsam8(t_hist s, uint16_t* mem,  std::string& disbuf, std::string&combuf) {
    ...
    int two_word=0;
         int	eff_addr = ea (s.pc, s.ir, ind, cur);
    switch (s.ir & 07000) {
        case	00000:			// AND
        case	01000:			// TAD
        case	02000:			// ISZ
